...
# Model comparisons for statistics of main effects and two-way interactions of cognitive load, surprisal, and age
```{r}
#---- Time Scale 1 ----
## two-way interactions
m_RT_ts1_twoway <- lmer(log(duration) ~ # control carry-over effects from previous trials:
                                                           previous_reading_times_log_centered +
                                                           # control diff. response strategies 
                                                           # on block- and participant-level:
                                                           dprime_centered + 
                                                           ...
                                                           # Surprisal on TS 1, age & cognitive load condition:
                                                           # main effects & their 2-way interactions 
                                                           surprisal_1_centered * age_centered + 
                                                           surprisal_1_centered * cognitive_load + 
                                                           ...
                                                           # complex models) and relatively conservative.
                                                           lmerControl(optimizer = "bobyqa"))
saveRDS(m_RT_ts1_twoway, file = "Analysis/RData/m_RT_ts1_twoway.rds")

# now use drop1 to calculate test statistic for interaction terms
res_ts1_twoway <- drop1(m_RT_ts1_twoway, test = "Chisq")

## main effects
m_RT_ts1_me <- lmer(log(duration) ~ # control carry-over effects from previous trials:
                                                           previous_reading_times_log_centered +
                                                           # control diff. response strategies 
                                                           # on block- and participant-level:
                                                           dprime_centered + 
                                                           ...
                                                           # complex models) and relatively conservative.
                                                           lmerControl(optimizer = "bobyqa"))
saveRDS(m_RT_ts1_me, file = "Analysis/RData/m_RT_ts1_me.rds")

# now use drop1 to calculate test statistic for main effects
res_ts1_me <- drop1(m_RT_ts1_me, test = "Chisq")

# Example to calculate estimated marginal means and to plot them for age and ts1:
emm_ts1 <- ggeffects::ggemmeans(m_RT_ts1_me, terms = "age_centered")
plot(emm_ts1, limit.range = TRUE)
```
...
# TS 1: first model with centered age, second model with uncentered age for easier interpretation:
...
# TS 4: first model with centered age, second model with uncentered age for easier interpretation:
...
# TS 12: first model with centered age, second model with uncentered age for easier interpretation:
...
# TS 60: first model with centered age, second model with uncentered age for easier interpretation:
...
# I'll use the models where age is not centered.
...
tab_model(m_RT_ts1_No_centering_age_random_slope, 
          m_RT_ts4_No_centering_age_random_slope, 
          m_RT_ts12_No_centering_age_random_slope,
          m_RT_ts60_No_centering_age_random_slope,
          seed = 42,
          show.se = TRUE, 
          show.stat = FALSE,
          show.intercept = FALSE, 
...
# get LMM results with p-values
summary_output <- Anova(m_RT_ts1_No_centering_age_random_slope, 
...
summary_output$`Pr(>Chisq)` <- p_values_fdr
print(summary_output)
print(summary(m_RT_ts1_No_centering_age_random_slope))

# Print model fits:
message("model fit for LMM of TS1:")
get_gof(m_RT_ts1_No_centering_age_random_slope)
message("model fit for LMM of TS4:")
get_gof(m_RT_ts4_No_centering_age_random_slope)
message("model fit for LMM of TS12:")
get_gof(m_RT_ts12_No_centering_age_random_slope)
message("model fit for LMM of TS60:")
get_gof(m_RT_ts60_No_centering_age_random_slope)
...
# For TS1
marginal_effects_ts1 <- plot_model(m_RT_ts1_No_centering_age_random_slope, 
                                    type = "pred", 
...
# For TS4
marginal_effects_ts4 <- plot_model(m_RT_ts4_No_centering_age_random_slope, 
                                    type = "pred", 
...
# For TS12
marginal_effects_ts60 <- plot_model(m_RT_ts12_No_centering_age_random_slope, 
                                    type = "pred", 
...
# For TS60
marginal_effects_ts60 <- plot_model(m_RT_ts60_No_centering_age_random_slope, 
                                    type = "pred", 
...
#####PLOT 2-WAY INTERACTION OF AGE & COGNITIVE LOAD ###############

# TS 1:
# Calculate estimated marginal means for the interaction effects:
int_eff_cognload_age <- effect("age:cognitive_load", m_RT_ts1_No_centering_age_random_slope)
# plot them:
plot_int_age_cognload_ts1 <- plot(int_eff_cognload_age, 
                              confint = list(style="auto"), # add CIs for each line
                              main = "Age x Cognitive Load - TS 1",
                              xlab = "Age (centered)",
...
# TS 4:
# Calculate estimated marginal means for the interaction effects:
int_eff_cognload_age <- effect("age:cognitive_load", m_RT_ts4_No_centering_age_random_slope)
# plot them:
plot_int_age_cognload_ts4 <- plot(int_eff_cognload_age, 
                              confint = list(style="auto"), # add CIs for each line
                              main = "Age x Cognitive Load - TS 4",
                              xlab = "Age (centered)",
                              ylab = "log reading time",
                              ylim = c(5.5, 8),
                              type = "response", 
...
# TS 12:
# Calculate estimated marginal means for the interaction effects:
int_eff_cognload_age <- effect("age:cognitive_load", m_RT_ts12_No_centering_age_random_slope)
# plot them:
plot_int_age_cognload_ts12 <- plot(int_eff_cognload_age, 
                              confint = list(style="auto"), # add CIs for each line
                              main = "Age x Cognitive Load - TS 12",
                              xlab = "Age (centered)",
                              ylab = "log reading time",
                              ylim = c(5.5, 8),
                              type = "response", 
...
# TS 60:
# Calculate estimated marginal means for the interaction effects:
int_eff_cognload_age <- effect("age:cognitive_load", m_RT_ts60_No_centering_age_random_slope)
# plot them:
plot_int_age_cognload_ts60 <- plot(int_eff_cognload_age, 
                              confint = list(style="auto"), # add CIs for each line
                              main = "Age x Cognitive Load - TS 60",
                              xlab = "Age (centered)",
                              ylab = "log reading time",
                              ylim = c(5.5, 8),
                              type = "response", 
...
plot_int_TS1 <- plot_model(m_RT_ts1_No_centering_age_random_slope,
                                     type = c("eff"), # plot effect
                                     terms = c("surprisal_1_centered", "cognitive_load"), 
                                     title = "TS1",
                                     show.legend = FALSE,
                                     line.size = 2,
                                     ci.lvl = 0.95, # plot 95% CI as shaded area around mean
                                     alpha = 0.4, 
                                     colors = palet_load) + 
                            ...
                            theme(panel.grid.minor = element_blank(), 
                                  ...
                                  axis.ticks = element_line(color = "black"),
                                  axis.line = element_line(color = "black")) + 
                            # hack y-axis into log-format:
                            coord_cartesian(ylim = c(log(400), log(1400))) +
                            scale_y_continuous(breaks = c(log(400), log(600), 
                                                          log(800), log(1000), 
                                                          log(1200), log(1400)), 
...
plot_int_TS4 <- plot_model(m_RT_ts4_No_centering_age_random_slope,
                                     type = c("eff"), # plot effect
                                     terms = c("surprisal_4_centered", "cognitive_load"), 
                                     title = "TS4",
                                     show.legend = FALSE,
                                     line.size = 2,
                                     ci.lvl = 0.95, # plot 95% CI as shaded area around mean
...
# TS 12:
plot_int_TS12 <- plot_model(m_RT_ts12_No_centering_age_random_slope,
                                     type = c("eff"), # plot effect
                                     terms = c("surprisal_12_centered", "cognitive_load"), 
                                     title = "TS12",
                                     show.legend = FALSE,
                                     line.size = 2,
                                     ci.lvl = 0.95, # plot 95% CI as shaded area around mean
...
# TS 60:
plot_int_TS60 <- plot_model(m_RT_ts60_No_centering_age_random_slope,
                                     type = c("eff"), # plot effect
                                     terms = c("surprisal_60_centered", "cognitive_load"), 
                                     title = "TS60",
                                     show.legend = FALSE,
                                     ci.lvl = 0.95, # plot 95% CI as shaded area around mean
                                     line.size = 2,
...
plot_age_TS1 <- plot_model(m_RT_ts1_No_centering_age_random_slope,
                                     type = c("eff"), # plot effect
                                     terms = c("age"), 
                                     title = "Age (TS1)",
                                     show.legend = FALSE,
...
plot_load_TS1 <- plot_model(m_RT_ts1_No_centering_age_random_slope,
                                     type = c("eff"), # plot effect
                                     terms = c("cognitive_load"), 
                                     title = "Cognitive Load (TS1)",
                                     show.legend = FALSE,
                                     line.size = 2,
                                     alpha = 0.4, 
                                     ci.lvl=0.95, # plot 95% CI as whiskers
                                     colors = palet_load) + # only shows first colour?!
                            labs(x = "Cognitive Load", y = NULL) +
                            theme_minimal() + 
                            theme(panel.grid.minor = element_blank(), 
                                  panel.grid.major = element_blank(),
                                  axis.ticks = element_line(color = "black"),
...
plot_surprisal_TS1 <- plot_model(m_RT_ts1_No_centering_age_random_slope,
                                     type = c("eff"), # plot effect
                                     terms = c("surprisal_1_centered"), 
                                     title = "Surprisal (TS1)",
                                     show.legend = FALSE,
                                     ...
                                     line.size = 2,
                                     alpha = 0.4, 
                                     colors = palet_effects) + # only shows first colour?!
                            labs(x = "Surprisal (centered)", y = NULL) +
                            theme_minimal() + 
                            theme(panel.grid.minor = element_blank(), 
                                  panel.grid.major = element_blank(),
                                  axis.ticks = element_line(color = "black"),
...
# also get main effects for surprisal on different TSs
# --> use the models with scrambled time scales here:
main_eff_TS1 <- effect("surprisal_1_centered", m_RT_ts1_No_centering_age_random_slope)
main_eff_TS4 <- effect("surprisal_4_centered", m_RT_ts4_No_centering_age_random_slope)
main_eff_TS12 <- effect("surprisal_12_centered", m_RT_ts12_No_centering_age_random_slope)
main_eff_TS60 <- effect("surprisal_60_centered", m_RT_ts60_No_centering_age_random_slope)
...
# Effects we just controlled for:

# estimate marginal means:
# --> use the model of TS 1 with all uncentered age variable here:
main_eff_dprime_TS1 <- effect("dprime_centered", m_RT_ts1_No_centering_age_random_slope)
main_eff_comprQs_mean_TS1 <- effect("compr_Qs_percent_correct_mean_centered", m_RT_ts1_No_centering_age_random_slope)
main_eff_comprQs_diff_TS1 <- effect("compr_Qs_percent_correct_diff_centered", m_RT_ts1_No_centering_age_random_slope)
main_eff_reaction_TS1 <- effect("reaction", m_RT_ts1_No_centering_age_random_slope)
main_eff_cognload_TS1 <- effect("cognitive_load", m_RT_ts1_No_centering_age_random_slope)
main_eff_block_nr_TS1 <- effect("block_nr", m_RT_ts1_No_centering_age_random_slope)
main_eff_trial_nr_TS1 <- effect("trial_nr", m_RT_ts1_No_centering_age_random_slope)
main_eff_word_length_TS1 <- effect("word_length_single_centered", m_RT_ts1_No_centering_age_random_slope)
main_eff_word_freq_TS1 <- effect("word_frequency_centered", m_RT_ts1_No_centering_age_random_slope)


# On block-level, reading gets a lot slower with increasing d-prime.
plot_main_eff_dprime <- plot(x = main_eff_dprime_TS1,
...
plot_main_eff_comprQs_mean <- plot(x = main_eff_comprQs_mean_TS1,
                              confint = list(style = "auto"), # add CIs
...
plot_main_eff_comprQs_diff <- plot(x = main_eff_comprQs_diff_TS1,
                              confint = list(style = "auto"), # add CIs
...
# "Reading" gets a lot slower when there's an n-back reaction:
plot_main_eff_reaction <- plot(x = main_eff_reaction_TS1,
                           confint = list(style = "auto"), # add CIs
                           main = "Main Effect 'Reaction'",
                           xlab = "Reaction",
                           ylab = "log reading times",
                           ylim = c(6, 7.1),
                           type = "response",
...
plot_main_eff_cognload <- plot(x = main_eff_cognload_TS1,
                               confint = list(style = "auto"), # add CIs
...
# Reading gets a little faster towards the end of the experiment:
plot_main_eff_block_nr <- plot(x = main_eff_block_nr_TS1,
                                 confint = list(style = "auto"), # add CIs
                                 main = "Main Effect 'Block Nr'",
                                 xlab = "Number of Block",
                                 ylab = "log reading times",
                                 ylim = c(6, 7.1),
                                 type = "response",
                                 rug = FALSE,
...
# Reading gets a little faster towards the end of the block:
plot_main_eff_trial_nr <- plot(x = main_eff_trial_nr_TS1,
                               confint = list(style = "auto"), # add CIs
                               main = "Main Effect 'Trial Nr'",
                               xlab = "Position of Word in the Block",
                               ylab = "log reading times",
                               ylim = c(6, 7.1),
                               type = "response",
                               rug = FALSE,
...
# I can't really see an effect of word frequency on reading speed 
# (it's not significant in the model, so this makes sense)
plot_main_eff_word_freq <- plot(x = main_eff_word_freq_TS1,
                               confint = list(style = "auto"), # add CIs
                               main = "Main Effect 'Word Frequency'",
                               xlab = "Word Frequency",
                               ylab = "log reading times",
                               ylim = c(6, 7.1),
                               type = "response",
...
# Reading gets a little slower with increasing word length:
plot_main_eff_word_length <- plot(x = main_eff_word_length_TS1,
                                confint = list(style = "auto"), # add CIs
                                main = "Main Effect 'Word Length'",
                                xlab = "Word Length",
                                ylab = "log reading times",
                                ylim = c(6, 7.1),
                                type = "response",
...
# Plot main effects for Surprisal on all TSs:

# Reading gets a little slower with increasing word surprisal:
plot_main_eff_TS1 <- plot(x = main_eff_TS1,
                                  confint = list(style = "auto"), # add CIs
                                  main = "Main Effect 'Surprisal TS1'",
                                  xlab = "Surprisal",
                                  ylab = "log reading times",
                                  ylim = c(6, 7.1),
                                  type = "response",
...
# Reading gets a little slower with increasing word length:
plot_main_eff_TS4 <- plot(x = main_eff_TS4,
                                  confint = list(style = "auto"), # add CIs
                                  main = "Main Effect 'Surprisal TS4'",
                                  xlab = "Surprisal",
                                  ylab = "log reading times",
                                  ylim = c(6, 7.1),
                                  type = "response",
...
# Reading gets a little slower with increasing word length:
plot_main_eff_TS12 <- plot(x = main_eff_TS12,
                                  confint = list(style = "auto"), # add CIs
                                  main = "Main Effect 'Surprisal TS12'",
                                  xlab = "Surprisal",
                                  ylab = "log reading times",
                                  ylim = c(6, 7.1),
                                  type = "response",
...
# Reading gets a little slower with increasing word length:
plot_main_eff_TS60 <- plot(x = main_eff_TS60,
                                  confint = list(style = "auto"), # add CIs
                                  main = "Main Effect 'Surprisal TS60'",
                                  xlab = "Surprisal",
                                  ylab = "log reading times",
                                  ylim = c(6, 7.1),
                                  type = "response",
