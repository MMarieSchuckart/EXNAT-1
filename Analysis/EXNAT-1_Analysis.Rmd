...
pkgs <- c("here", # for working with relative paths
          ...
          "Hmisc", # for plotting CI in spaghetti plots of RTs
...
# save df
save(df_clean, file = here::here("Analysis/Analysis/RData/df_clean_all_subjects_09_24.RData"))
...
plot_age
# save plot:
ggsave(plot_age, file = here::here(paste0("Analysis/Plots/age_distr_", today, ".pdf")), width = 5, height = 5)


```


```{r plot RT by Condition & Age & Location (only dual-task blocks), message = FALSE, warning = FALSE}
...
rt_age_log <- ggplot(plot_df, aes(x = Age, y = Reaction_time, color = Condition))+
              geom_point2(alpha = 0.5, size = 1.3) +
...
ggsave(rt_age_log, file = here::here(paste0("Analysis/Plots/RT_age_cond_log_all_", today, ".pdf")), 
       width = 8, height = 5)  




# Also plot RTs by condition as spaghetti plot:
spaghetti_plot_RTs_cond <- ggplot(plot_df, aes(x = Condition, y = Reaction_time, group = Subject)) +
  geom_line(alpha = 0.3, color = "grey") +  # Individual participant lines with transparency
  geom_point(alpha = 0.5, size = 1.5, aes(color = Condition)) +  # Points at each condition
  stat_summary(aes(group = 1), fun.data = mean_cl_boot, geom = "ribbon", 
               alpha = 0.8, fill = "white", color = "grey") + # CI ribbon around the mean line
  stat_summary(aes(group = 1), fun = mean, geom = "line", color = "black", size = 1.2) +  # Mean line across participants
  stat_summary(aes(group = 1), fun = mean, geom = "point", color = "black", size = 2.5) + # Mean points at each condition
  scale_color_manual(values = palet_load, guide = "none") + # Hide individual participant legend
                # make y-axis log-transformed (with log base e as in log() function)
              scale_y_log(breaks=c(0, 250, 500, 1000, 2000, 4000), 
                          labels=c("0","250", "500", "1000", "2000", "4000")) +
  labs(x = "Condition", y = "Reading Time (in ms)", title = "RTs by Condition") +
  apatheme + # Apply custom theme
  theme(legend.position = "none") # Remove legend

ggsave(spaghetti_plot_RTs_cond, file = here::here(paste0("Analysis/Plots/RT_cond_all_spaghetti.pdf")), 
       width = 8, height = 4.5)  
...
ggsave(rt_age_log_location, 
       file = here::here(paste0("Analysis/Plots/RT_age_cond_log_location_", today, ".pdf")), 
       width = 8, height = 5)  
```


```{r Plot d-primes by Condition, Location & Age, echo = TRUE}

# Hint: We didn't have an n-back task in the baseline condition, so we can only 
# compute & plot them for the 1-back and 2-back dual- and single-task blocks.
...
ggsave(plot_dprimes, file = here::here(paste0("Analysis/Plots/dprimes_", today, ".pdf")), 
...
ggsave(plot_dprimes_location, file = here::here(paste0("Analysis/Plots/dprimes_location_", today, ".pdf")), 
...
```{r Plot Comprehension Question Performance by Condition & Age & Location, echo = FALSE}
...
ggsave(plot_comprehensionQs, 
       file = here::here(paste0("Analysis/Plots/compr_Qs_", today, ".pdf")), 
       device = "pdf", width = 4, height = 4)
...
ggsave(plot_comprehensionQs_location, 
       file = here::here(paste0("Analysis/Plots/compr_Qs_location_", today, ".pdf")), 
       device = "pdf", width = 8, height = 4)


```

# Additional plots: Plot RT by Task
...
plot_RT_by_task_location <- ggplot(plot_df, aes(x = Age, 
                              ...
                              theme(legend.position = "bottom")
ggsave(plot_RT_by_task_location, file = here::here(paste0("Analysis/Plots/RT_by_task_location_", today, ".pdf")), 
       device = "pdf", width = 8, height = 4)
...
plot_RT_by_task <- ggplot(plot_df, aes(x = Age, 
                   ...
                   theme(legend.position = "bottom")
plot_RT_by_task
ggsave(plot_RT_by_task, file = here::here(paste0("Analysis/Plots/RT_by_task_", today, ".pdf")), 
       device = "pdf", width = 4, height = 4)
...
# Stats
```{r prepare data for LMMs, echo = FALSE}
...
# compare LMM 3-way interaction slopes between ages
``` {r compare LMM 3-way interaction slopes between ages}

# exploring 3-way interaction age, surprisal, and cognitive load
# compare slopes of 18- and 85-year old person to see whether they are significantly different from each other
emtrends(m_RT_ts2_age_centered_random_slope_entropy, pairwise ~ age_centered | cognitive_load, var = "surprisal_2_centered", at = list(age_centered=c(18, 85)))

# testing differences in cognitive load slopes for specific ages:
# YA
emtrends(m_RT_ts2_age_centered_random_slope_entropy, pairwise ~ cognitive_load, var = "surprisal_2_centered", at = list(age_centered=c(18)), adjust = "fdr") 

# OA
emtrends(m_RT_ts2_age_centered_random_slope_entropy, pairwise ~ cognitive_load, var = "surprisal_2_centered", at = list(age_centered=c(85)), adjust = "fdr") 
```



``` {r LMM 3-way interaction barplot insets}
summary_emtrends<- emtrends(m_RT_ts2_age_centered_random_slope_entropy, 
                            pairwise ~ age_centered |cognitive_load, # group by age
                            var = "surprisal_2_centered", 
                            at = list(age_centered = c(27, 45, 63)))

LMM_3way_surprisal_estimates_summary <- summary(summary_emtrends$emtrends)[c(1, 2, 3, 4)]

# Now plot it!
barplot_inset_3way <- ggplot(LMM_3way_surprisal_estimates_summary, aes(x = cognitive_load, y = surprisal_2_centered.trend, 
                                                 fill = cognitive_load, group = age_centered)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8), color = "black") +
  geom_errorbar(aes(ymin = surprisal_2_centered.trend - SE, ymax = surprisal_2_centered.trend + SE), 
                position = position_dodge(width = 0.8), width = 0.25) +
  facet_wrap(~ age_centered, scales = "free_y") +  # Separate plots by age group
  scale_fill_manual(values = palet_load) +  # Colors for cognitive load conditions
  labs(x = "Cognitive Load", y = "Effect of Surprisal (ß)", 
       title = "Effect of Surprisal by Age and Cognitive Load Condition") +
  scale_y_continuous(limits = c(0.0, 0.01)) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# save df:
ggsave(barplot_inset_3way, file = here::here(paste0("Analysis/Plots/barplot_inset_3way_", today, ".pdf")), 
...
``` {r LMM 2-way interaction barplot insets}

# First inset: Load x Surprisal:
summary_emtrends <- emtrends(m_RT_ts2_age_centered_random_slope_entropy, 
                            pairwise ~ cognitive_load | surprisal_2_centered, # group by load
                            var = "surprisal_2_centered")

LMM_2way_surprisal_estimates_summary <- summary(summary_emtrends$emtrends)[c(1,3,4)]

# Now plot it!
barplot_inset_surprisal_2way <- ggplot(LMM_2way_surprisal_estimates_summary, aes(x = cognitive_load, y = surprisal_2_centered.trend, 
                                                 fill = cognitive_load)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8), color = "black") +
  geom_errorbar(aes(ymin = surprisal_2_centered.trend - SE, ymax = surprisal_2_centered.trend + SE), 
                position = position_dodge(width = 0.8), width = 0.25) +
  scale_fill_manual(values = palet_load) +  # Colors for cognitive load conditions
  labs(x = "Cognitive Load", y = "Effect of Surprisal (ß)", 
       title = "Effect of Surprisal by Cognitive Load Condition") +
  scale_y_continuous(limits = c(0.0, 0.013)) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# save df:
ggsave(barplot_inset_surprisal_2way, file = here::here(paste0("Analysis/Plots/barplot_inset_surprisal_2way_", today, ".pdf")), 
       device = "pdf", width = 5, height = 2)




# Second inset: Load x Age:

summary_emtrends <- emtrends(m_RT_ts2_age_centered_random_slope_entropy, 
                            pairwise ~ cognitive_load | age_centered, # group by load
                            var = "age_centered")

LMM_2way_age_estimates_summary <- summary(summary_emtrends$emtrends)[c(1,3,4)]

# Now plot it!
barplot_inset_age_2way <- ggplot(LMM_2way_age_estimates_summary, aes(x = cognitive_load, y = age_centered.trend, 
                                                 fill = cognitive_load)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8), color = "black") +
  geom_errorbar(aes(ymin = age_centered.trend - SE, ymax = age_centered.trend + SE), 
                position = position_dodge(width = 0.8), width = 0.25) +
  scale_fill_manual(values = palet_load) +  # Colors for cognitive load conditions
  labs(x = "Cognitive Load", y = "Effect of Surprisal (ß)", 
       title = "Effect of Surprisal by Cognitive Load Condition") +
...
# save df:
ggsave(barplot_inset_age_2way, file = here::here(paste0("Analysis/Plots/barplot_inset_age_2way_", today, ".pdf")), 
       device = "pdf", width = 5, height = 2)
...
length(as.numeric(subset(df_demogr, excl == F)$age))
...
# We are fitting a generalized additive model (GAM) with approx. the same random structure like our LMM, 
# except we are now adding smoothing splines, accounting for the possibility that 
# continuous predictors might have a non-linear relationship with our outcome variable
...
# predict slopes for three-way interaction, this gives suprisal plotted at mean +- SD
pred <- predict_response(m_gam_RT_ts2, terms = c("age", "surprisal_2", "cognitive_load"))
plot(pred)

# calculate simple slopes for three-way interaction and plot them
...
# plot the slopes by load condition
plot(slopes) + 
...
ggsave(plot = marginal_effects_dprime_age_load, 
       filename = here::here(paste0("Analysis/Plots/marginal_effects_dprimes_age_load_", today, ".pdf")),         
...
ggsave(plot = marginal_effects_comprehension_Qs_age_load, 
       filename = here::here(paste0("Analysis/Plots/marginal_effects_comprehension_Qs_age_load_", today, ".pdf")),         
       width = 5, height = 6, device = "pdf")
...
ggsave(plot = marginal_effects_3way_interaction, 
       filename = here::here(paste0("Analysis/Plots/marginal_effects_3way_interaction_", today, ".pdf")),  
...
ggsave(plot = plot_int_surprisal_load, 
       filename = here::here(paste0("Analysis/Plots/marginal_effects_surprisal_load_", today, ".pdf")),  
       width = 7, height = 5, device = "pdf")
...
ggsave(plot = plot_int_age_load, 
       filename = here::here(paste0("Analysis/Plots/marginal_effects_age_load_", today, ".pdf")),  
       width = 7, height = 5, device = "pdf")
...
ggsave(plot = plot_int_age_surprisal, 
       filename = here::here(paste0("Analysis/Plots/marginal_effects_age_surprisal_", today, ".pdf")),  
       width = 7, height = 5, device = "pdf")
...
ggsave(plot = marginal_effects_age_surprisal_load, 
       filename = here::here(paste0("Analysis/Plots/marginal_effects_age_surprisal_load_", today, ".pdf")),  
...
##### PLOT PREDICTED DATA FOR MAIN EFFECTS ###############

# Plot predicted data points for main effect of age
plot_age_id <- plot(predict_response(m_RT_ts2_age_centered_random_slope_entropy, 
                                     terms = c("age_centered")), 
                    collapse_group = "ID", 
                    colors = palet_effects, 
                    line.size = 1,
                    ...
                    scale_x_continuous(breaks = c(0 - mean_age,
                                                  20 - mean_age, 
                                                  40 - mean_age,
                                                  60 - mean_age,
                                                  80 - mean_age), 
...
ggsave(plot = plot_age_id, 
       filename = here::here(paste0("Analysis/Plots/marginal_effects_age_by_ID_", today, ".pdf")),  
       width = 6, height = 5, device = "pdf")

# Cognitive Load
pred <- predict_response(m_RT_ts2_age_centered_random_slope_entropy, 
                      terms = c("cognitive_load"))

plot_load_id <- plot(predict_response(m_RT_ts2_age_centered_random_slope_entropy, 
                      terms = c("cognitive_load")), 
                     collapse_group = "ID", 
                     dot_size = 1, 
                     line_size = 1,
                ...
                labs(x = "Cognitive Load", y = "Reading Time (in ms)") +
                theme_minimal() + 
                scale_color_manual(values = palet_load, guide = "none") +
                theme(panel.grid.minor = element_blank(), 
                      panel.grid.major = element_blank(),
                      axis.ticks = element_line(color = "black"),
                      axis.line = element_line(color = "black")) + 
...
ggsave(plot = plot_load_id, 
       filename = here::here(paste0("Analysis/Plots/marginal_effects_load_by_ID_", today, ".pdf")),  
       width = 5, height = 5, device = "pdf")

# Surprisal
plot(predict_response(m_RT_ts2_age_centered_random_slope_entropy, 
                      terms = c("surprisal_2_centered")), 
     collapse_group = "ID", 
     dot_size = 1, 
     line.size = 1,
...
# Surprisal and Cognitive Load
plot(predict_response(m_RT_ts2_No_centering_age_random_slope_entropy, terms = c("surprisal_2_centered", "cognitive_load")), collapse_group = "ID", dot_size = 0.6, dot_alpha= 0.01, alpha = 0.3, colors = palet_load)


summary(surprisal_df$surprisal_2_words_context)
...
#ggsave(spaghetti_plot_RTs_cond, file = here::here(paste0("Plots/RT_cond_all_spaghetti.pdf")), 
#       width = 8, height = 4.5) 
...
ggsave(plot = simslopes, 
       filename = here::here(paste0("Analysis/Plots/simple_slopes_", today, ".pdf")),
