...
# Script for Simple Slopes Analysis for Timescale 60
...
## load model for ts60
m_RT_ts60_No_centering_age_random_slope <- readRDS(here::here("Analysis/RData/Mixed_models/m_RT_ts60_No_centering_age_random_slope.rds"))
...
ss_RT_ts60_noCentering_full_random_structure <- sim_slopes(m_RT_ts60_No_centering_age_random_slope, pred = surprisal_60_centered, modx = age, mod2 = cognitive_load, 
                                                           jnplot = T, data = lmm_df, sig.color = "#023FA5", insig.color = "#8E063B", 
                                                           control.fdr = T, mod.range = c(18,85), r.squared = F)
save(ss_RT_ts60_noCentering_full_random_structure, file = here::here(paste0("Analysis/RData/Mixed_models/simslopes_ts60_", today, ".RData")))

# Plot simple slopes
## load individual plots into variables
# baseline <- ss_RT_ts60_noCentering_full_random_structure$jn[[1]]$plot
# one_back <- ss_RT_ts60_noCentering_full_random_structure$jn[[2]]$plot
# two_back <- ss_RT_ts60_noCentering_full_random_structure$jn[[3]]$plot
# 
# ## merge all plots together into one column
# simslopes_ts60 <- ggarrange(baseline +
...
#                               labs(title = NULL), 
#                             ncol = 1)
# ggsave(plot = simslopes_ts60, file = "test_ts60.pdf")
# #ggsave(plot = simslopes_ts1, 
# #       filename = here::here(paste0("Analysis/RData/Mixed_models/simslopes_RT_ts1_", today, ".pdf")), dpi = 300, width = 10, height = 12, device = "pdf")