...
# Script for Simple Slopes Analysis for Timescale 1
...
## load model for ts1
m_RT_ts1_No_centering_age_random_slope <- readRDS(here::here("Analysis/RData/Mixed_models/m_RT_ts1_No_centering_age_random_slope.rds"))
...
ss_RT_ts1_noCentering_full_random_structure <- sim_slopes(m_RT_ts1_No_centering_age_random_slope, pred = surprisal_1_centered, modx = age, mod2 = cognitive_load, 
                                                          jnplot = T, data = lmm_df, sig.color = "#023FA5", insig.color = "#8E063B", 
                                                          control.fdr = T, mod.range = c(18,85), r.squared = F)
save(ss_RT_ts1_noCentering_full_random_structure, file = here::here(paste0("Analysis/RData/Mixed_models/simslopes_ts1_", today, ".RData")))
